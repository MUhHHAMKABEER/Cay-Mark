...
<script>
    ...
    function applyDecodedData(data) {
        ...
        // Case-insensitive: find the <option> whose value OR display text matches decodedVal
        ...
        Object.keys(data || {}).forEach(function(key) {
            if (SKIP[key]) return; // island and other protected fields
            ...
            if (el.tagName === 'SELECT') {
                ...
                if (matched) {
                    ...
                    // Refresh the portal-based custom dropdown trigger via public API
                    if (typeof window.cmSelRefreshTrigger === 'function') {
                        window.cmSelRefreshTrigger(el);
                    }
                }
                // No match → leave field at placeholder "Select"
            } else {
                el.value = data[key];
            }
        });
